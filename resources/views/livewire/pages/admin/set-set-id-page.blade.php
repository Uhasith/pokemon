...
use App\Imports\SetSetIdImport;
...
new #[Layout('layouts.admin')] class extends Component {
    ...
    public function mount()
    {
        ...
        $this->dbColumns = config('pokemon.setSetIdTableColumns');
    }
    ...
    public function submit()
    {
        ...
        try {
            ...
            if (!isset($this->columnMappings['set_id']) || empty($this->columnMappings['set_id']) || !isset($this->columnMappings['related_set_id']) || empty($this->columnMappings['related_set_id'])) {
                app(NotificationService::class)->sendExeptionNotification("The 'Set Id' and 'Related Set Id' column is required.");
                return;
            }

            Log::info('Importing cards...');

            Excel::import(new SetSetIdImport($this->columnMappings), $this->file);
...
<div class="py-4">
    <div class="max-w-7xl mx-auto flex gap-4 justify-end sm:px-6 lg:px-8 mb-2">
        <x-wui-mini-button info icon="document-arrow-down" x-on:click="$openModal('cardtcgModal')"
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg min-h-[70vh]">
            <div class="p-6 text-gray-900 dark:text-gray-100">
               <livewire:tables.set-set-id-table/>
            </div>
        </div>
        <div>
            <x-wui-modal-card title="Import Set and Related Set" name="cardtcgModal" wire:model.live="setsetIdModalState" width="5xl"
                align="center">
                        ...
                        @if ($this->file && count($this->excelColumns) > 0)
                            <div class="grid grid-cols-2 gap-4">
                                @foreach ($this->dbColumns as $index => $dbColumn)
                                    <div class="grid grid-cols-[1fr_2fr] items-center gap-4 py-4" wire:key="set-set-id-pg-db-col-{{$index}}">
