...
use App\Imports\CardTcgPcImport;
...
new #[Layout('layouts.admin')] class extends Component {
    ...
    public function mount()
    {
        ...
        $this->dbColumns = config('pokemon.cardTcgPcTableColumns');
    }
    ...
    public function submit()
    {
        ...
        try {
            ...
            if (!isset($this->columnMappings['card_id']) || empty($this->columnMappings['card_id']) || !isset($this->columnMappings['tcg_id']) || empty($this->columnMappings['tcg_id'])) {
                app(NotificationService::class)->sendExeptionNotification("The 'Card Id' and 'Tcg Id' column is required.");
                return;
            }

            Log::info('Importing cards...');

            Excel::import(new CardTcgPcImport($this->columnMappings), $this->file);
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg min-h-[70vh]">
            <div class="p-6 text-gray-900 dark:text-gray-100">
               <livewire:tables.card-tcg-pc-table/>
            </div>
        </div>
        <div>
            <x-wui-modal-card title="Import Cards" name="cardtcgModal" wire:model.live="cardtcgModalState" width="5xl"
                align="center">
                        ...
                        @if ($this->file && count($this->excelColumns) > 0)
                            <div class="grid grid-cols-2 gap-4">
                                @foreach ($this->dbColumns as $index => $dbColumn)
                                    <div class="grid grid-cols-[1fr_2fr] items-center gap-4 py-4" wire:key="card-tcg-pg-db-col-{{$index}}">
