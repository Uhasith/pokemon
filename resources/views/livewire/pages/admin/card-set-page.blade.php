...
new #[Layout('layouts.admin')] class extends Component {
    ...
    public function mount()
    {
        ...
        $this->dbColumns = config('pokemon.cardSetTableColumns');
    }
...
<div class="py-4">
    <div class="max-w-7xl mx-auto flex gap-4 justify-end sm:px-6 lg:px-8 mb-2">
        <x-wui-mini-button info icon="document-arrow-down" x-on:click="$openModal('cardSetModal')"
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg min-h-[70vh]">
            <div class="p-6 text-gray-900 dark:text-gray-100">
              <livewire:tables.card-set-table/>
            </div>
        </div>
        <div>
            <x-wui-modal-card title="Import Cards" name="cardSetModal" wire:model.live="cardSetModalState" width="5xl"
                align="center">
                        ...
                        @if ($this->file && count($this->excelColumns) > 0)
                            <div class="grid grid-cols-2 gap-4">
                                @foreach ($this->dbColumns as $index => $dbColumn)
                                    <div class="grid grid-cols-[1fr_2fr] items-center gap-4 py-4" wire:key="card-set-pg-db-col-{{$index}}">
